...
#regionVars:             [HT   ,      MET,        R,             Ntag,        dPhi,    Njets,            NElectrons,        NMuons,TrkPt,TrkEta,BinNumber ] 
		fv = [adjustedHt,adjustedMet.Pt(),R,1+len(disappearingTracks),dPhi,adjustedNJets, len(SmearedElectrons)-1, len(SmearedMuons), pt,eta]
	...
	if singleMuEvent_:
		...
		ptForKappa = pt
                R = 2*(abs(pt - adjustedMet.Pt())/(pt + adjustedMet.Pt()))
                dPhi =  math.acos(math.cos((adjustedMet.Phi()-phi)))
                fv = [adjustedHt,adjustedMet.Pt(),R,1+len(disappearingTracks),dPhi,adjustedNJets, len(SmearedElectrons), len(SmearedMuons)-1, pt,eta]
		fv.append(getBinNumber(fv))
	...
	if presentDisTrkEvent:
		...
		#print ientry, 'found a nice dt', dt.Pt()
                    ...
                    adjustedJets = []
                    adjustedHt = 0
                    adjustedMet = metvec # MET needs to be adjusted only in control region because we have to replace lep with disptrk(notpf).
                    adjustedMht = TLorentzVector()
                    adjustedMht.SetPxPyPzE(0,0,0,0)
                    for ijet, jet in enumerate(c.Jets):
			if not jet.Pt()>30: continue
			...
			eta = abs(dt.Eta())
                    ...
                    R = 2*(abs(pt - adjustedMet.Pt())/(pt + adjustedMet.Pt()))
                    dPhi =  math.acos(math.cos((adjustedMet.Phi()-phi)))
                    fv = [adjustedHt,adjustedMet.Pt(),R,len(disappearingTracks),dPhi,adjustedNJets, len(RecoElectrons), len(RecoMuons), pt,eta]
                    fv.append(getBinNumber(fv))
