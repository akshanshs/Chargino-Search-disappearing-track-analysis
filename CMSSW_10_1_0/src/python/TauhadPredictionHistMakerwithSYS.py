...
newfname = 'Tau_BkgPredictionwithSYSHists_'+identifier+'.root'
...
def getGenID(track):
    drsmall = 0.2
    gen_id = False
    for igen, gen in enumerate (c.GenTaus_LeadTrk):  #(c.GenParticles): update
     #   if not ((abs(c.GenParticles_PdgId[igen]) == 16)) : continue # use genId = 16 for tau neutrino as tau decays to tau neutrino + pions update
#        if not (abs(c.GenParticles_PdgId[igen]) == 1000024 and c.GenParticles_Status[igen] == 1): continue
        if c.GenTaus_had[igen] == 0: continue
...
#        if TriggerResult(MetTriggers) == False: continue
	...
	for itrack, track in enumerate(c.tracks):
		...
		if not getGenID(track): continue     #getGenID(track) returns false if track not matched to gen chargino # updated to for taus identification
                disappearingTracks.append(track)  # charge taken to be one as no branch  for charge.
    
        tauevent = len(disappearingTracks) > 0 

	if not (tauevent): continue
	...
	if tauevent:
		...
		fv.append(getBinNumber(fv))
                fv.append(nVtx)
                uncertainity = .99  # taking flat uncertanity
		k      = 1
                k_up   = 1 + uncertainity
                ...
                if k_up< 0: k_up = 0.99*k
                if k_down < 0: k_down =0.01*k
                if k_error < 0 :k_error = k

		for regionkey in regionCuts:
                    for ivar, varname in enumerate(varlist_):
                        hname = 'Tau'+regionkey+'_'+varname
