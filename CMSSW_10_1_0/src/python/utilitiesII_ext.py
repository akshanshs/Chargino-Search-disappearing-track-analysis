...
def FabDrawPrediction(cGold,legbkg, legsig,hData,hbkgstack,hsigstack,hbkg,var,syshist,lumi=35.9,LinearScale=False, fraction = '#frac{prediction}{data}'):
	cGold.cd()
...
#	hRatio.Divide(hDataCopy)
	...
	histratio = hData.Clone('histratio')
	hbkgforsys = hbkg.Clone('hbkgforsys')
	syshistcopy = syshist.Clone('syshistcopy')
	histratio.Add(hbkgforsys, -1)
	histratio.Divide(syshistcopy)
	for ibin in range(1,histratio.GetXaxis().GetNbins()+1): histratio.AddBinContent(ibin,1) 
...
#	histratio.Draw('histsame')
#	histratio.Draw('e0')
	cGold.Update()
	return hRatio,hbkg_,histratio
