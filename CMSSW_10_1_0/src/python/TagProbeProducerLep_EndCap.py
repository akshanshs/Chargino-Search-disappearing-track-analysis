...
fSmear  = TFile(fname)
keys = fSmear.GetListOfKeys()
sfHist  = fSmear.Get('htrkresp')   #htrkrespL

TemplateBinList = [[0,1.4442,0,80],[0,1.4442,80,999999],[1.566,2.4,0,80],[1.566,2.4,80,999999]]
# TemplateBinList = [no bin, [eta1,eta2,Pt1,Pt2],............]                                                                                                 

templateList = []
for key in keys:
        hist = key.GetName()
        template = fSmear.Get(hist)
	...
	for ientry in range(nentries):
		...
		for igen, gen in enumerate(c.GenParticles):
				if 1 < 2:
						...
						if not (abs(gen.Eta()) < 2.4 and abs(gen.Eta()) > 1.566): continue #< 1.4442): continue
			
						fillth1(hEleGenPt, gen.Pt(), weight)
						fillth1(hEleGenEta, abs(gen.Eta()), weight)
						for im, m in enumerate(c.Electrons):
								if not (m.Pt() > 10 and abs(m.Eta()) < 2.4 and abs(m.Eta()) > 1.566): continue#1.4442 ): continue
								drBig4Trk = 9999
								...
								if dr < drsmall:
										...
										idlep   = im
						if drsmall < .01:
							        sf = getSF(abs(c.Electrons[idlep].Eta()), c.Electrons[idlep].Pt())
						...
						for itrk, trk in enumerate(c.tracks):
								if not (abs(trk.Eta()) < 2.4 and abs(trk.Eta()) > 1.566): continue#< 1.4442): continue
								...
								if dr < drsmal:
										...
										dtTlvsum = trk
						if drsmal < .01:
								#print ientry, 'found dt', dtTlvsum.Pt()								
		...
		dIMmax = 999
		for itrack, track in enumerate(c.tracks):
				if not (abs(track.Eta()) < 2.4 and abs(track.Eta()) > 1.566): continue
		...
		for ireco, reco in enumerate(c.Electrons):
				if not (abs(reco.Eta()) < 2.4 and abs(reco.Eta()) > 1.566): continue#< 1.4442)): continue
...
#				sf = 10**(sfHist.GetRandom())
				sf = getSF(abs(c.Electrons[ireco].Eta()), c.Electrons[ireco].Pt())
			#	print sf
			#	pause()
...
def getSF(eta, Pt):
        print 'eta:',eta,'Pt',Pt
        for ibin, bin in enumerate(TemplateBinList):
                if eta > bin[0] and eta < bin[1] and Pt > bin[2] and Pt < bin[3]:
                        print bin
                        print templateList[ibin+1]
                        return 10**(templateList[ibin+1].GetRandom())
	return 1
