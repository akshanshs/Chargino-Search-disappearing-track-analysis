...
for key in keys:
    name = key.GetName()
    if not ('EtaDT_num' in name or 'PtDT_num' in name):continue
    hnum   = f.Get(name)
    ...
    if 'vs' in name: 
        ...
        hden.GetXaxis().SetTitle(namelib[name.replace('DT_num','').replace('DTmeff','').replace('EleProbe','').replace('h','').replace('EleGen','')])
    ...
    leg.Draw()                                                                                                                                                
    c1.Update()
    histname = 'kappa'+name.replace('DT_num','').replace('DTmeff','').replace('RECO_den','').replace('EleProbe','tagNprobe').replace('h','').replace('EleGen','GenInfo')#kappaGenPtvsEta
    print 'grabbing', name, 'creating', histname
    #pause()
