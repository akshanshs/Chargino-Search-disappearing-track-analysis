...
if inputFiles ==  []:
		...
		inputFiles = ["/pnfs/desy.de/cms/tier2/store/user/sbein/NtupleHub/Production2016v2/Summer16.DYJetsToLL_M-50_HT-100to200_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1_63_RA2AnalysisTree.root"]
		...
		for igen, gen in enumerate(c.GenParticles):
				if 1 < 2:
						...
						if drsmall < .01:
								...
								hEleGenPtRECOeff.Fill(smearedEleProbesum.Pt(), (c.CrossSection*35.9)/(1*.001))
						...
						#        smearedEleProbesum.SetPtEtaPhiE(sf*c.Electrons[idlep].Pt(),c.Electrons[idlep].Eta(),c.Electrons[idlep].Phi(),sf*c.Electrons[idlep].E())
								...
								hmuonresptest.Fill(math.log10(smearedEleProbesum.Pt()/gen.Pt()),(c.CrossSection*35.9)/(1*.001))
...
#                        print 'test IM e', reco.M()
				if (C1 + c.Electrons_charge[ireco] ==0):
						...
						hIMmuZ.Fill(tagProbeTlvSum.M(), (c.CrossSection*35.9)/(1*.001))
				sf = 10**(sfHist.GetRandom())
				...
				if (C1 + c.Electrons_charge[ireco] ==0):
						hIMmuZsmear.Fill(tagProbeTlvSum.M(), (c.CrossSection*35.9)/(1*.001))
...
#                #print 10*'*'
		if (IM > 60 and IM < 120  and (C1+C2 )==0):
				hIMZ.Fill(IM, (c.CrossSection*35.9)/(1*.001))##try to use this to get counts
				P2   = probeTlv.Pt()
...
#                        #print 'trk charge:', c.tracks_charge[itrack]
...
#                #print "track candidate number", itrack , "in event", ientry , "mu mu invariant mass =", IM
		#print 'tracks', IM
		#print 10*'*'
		if (IM > 60 and IM < 120 and (C1+C2 )==0):
				P2 = probeTlv.Pt()
