...
def main():
    ...
    if inputFiles == []:
        print 'running on default fast sim events'
#        inputFiles = ['/nfs/dust/cms/user/singha/LLCh/CMSSW_8_0_20/src/analysis/batch_test/pMSSM12_MCMC1_8_373637_step2_AODSIM_10of100.root']
        inputFiles = ['/nfs/dust/cms/user/singha/fastsim/fastSim_Jun/CMSSW_8_2_0/src/test55_LLP_AOD.root']
...
#basic track variables
    ...
    ch = 0
    hdr                 = TH1F("hdr", "DR(#chi,track)", 25, -0.0001, 2)
    decaylength         = TH1F("decaylength", "#chi^{#pm} log_{10}( d_{xy})", 25, -1, 4)
...
#######Fill the tree
                hdr.Fill(drmin)
                if drmin < 0.2: #and veto:
                    print 'disappearing track found'
...
#                    print 10*'*='
    ...
    print 'total number of charginos', ch
    ...
    f    = TFile('testValidation55.root','recreate')
...
#    fhist = TFile('hists_'+identifier+'.root','recreate')  # root tree for histograms
    fhist = TFile('hists_Validation55.root','recreate')
