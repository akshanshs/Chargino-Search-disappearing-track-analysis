...
#	if (len(SmearedElectrons)>0 and len(disappearingTracks)>0): continue	
	...
	if singleEleEvent_:
		...
		for jet in c.Jets:
			...
			if not jet.DeltaR(RecoElectrons[0])>0.04: continue
			adjustedMht-=jet
			adjustedNJets+=1
	...
	#	if abs(eta) > 1.4442 and abs(eta) < 1.566 : print abs(eta), 'eta of electron'
		...
		k = getKappa_(abs(eta),min(pt,700),SmearedElectrons[0][1])
		#print 'Kappa is', k
...
#	pause()
	if singleDisTrkEvent:
		...
		for jet in c.Jets:
			if not jet.Pt()>30: continue
			if not abs(jet.Eta())<5.0: continue
			if not jet.DeltaR(disappearingTracks[0])>0.04: continue	
			adjustedMht-=jet			
			adjustedNJets+=1	
	#	if not adjustedNJets>0: continue				
		if doGenVersion:
