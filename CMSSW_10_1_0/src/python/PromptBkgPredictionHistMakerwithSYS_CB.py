...
else: fileKappa = '/nfs/dust/cms/user/singha/LLCh/BACKGROUNDIII/CMSSW_10_1_0/src/Kappa_Run2016dm301p01p11p5rebin31Marbinextentionforfunction.root'
...
#            if (TriggerResult(ElectronTriggers) or TriggerResult(MuonTriggers) ) == False: continue
	...
	for iel, ele in enumerate(c.Electrons):
		...
		if not (smearedEl.Pt()> Pt_threshold and smearedEl.Pt()<2499): continue
                ...
                for ijet, jet in enumerate(c.Jets):
                    ...
                    if jet.DeltaR(ele) > 0.01 and jet.DeltaR(ele) < 0.4: ele_injet = ele_injet + 1
                if ele_injet > 0 : continue
	...
	for ilep, mu in enumerate(c.Muons):
		...
		if not (smearedMu.Pt()> Pt_threshold and smearedMu.Pt()<9999): continue
                muon_injet = 0    #########check if the muon is a jet i.e. a jet is identified as a muon
                for ijet, jet in enumerate(c.Jets):
                    if not c.Jets_ID[ijet]: continue
                    if jet.DeltaR(mu) > 0.001 and jet.DeltaR(mu) < 0.4: muon_injet = muon_injet + 1
                if muon_injet > 0 : continue
...
#					if debug : print 'filled hists for electrons'
	if singleMuEvent_:
		...
		k      = fetchKappa(abs(eta),min(ptForKappa,9999.99), kappadictMu)
                ...
                if k < 0: k =0
                if k_up< 0: k_up = 0
                if k_down < 0: k_down =0
                if k_error < 0 :k_error = 0
		for regionkey in regionCuts:
			for ivar, varname in enumerate(varlist_):
