...
def main():
    ...
    if inputFiles == []:
        ...
        inputFiles = ['/nfs/dust/cms/user/singha/FullSim/final_ext/CMSSW_8_0_21/src/AODSIM/g1800_chi1400_27_200970_step3_100AODSIM_74nFiles1.root']
#        inputFiles = ['/nfs/dust/cms/user/singha/LLCh/SigSamples/AODSIM1000CM/pMSSM12_MCMC1_27_200970_step3_AODSIM_1000_32nFiles1.root']
#        inputFiles = ['/nfs/dust/cms/user/singha/FullSim/final/CMSSW_8_0_21/src/AODSIM/pMSSM12_MCMC1_27_200970_step3_AODSIM_100_100nFiles1.root']
#        inputFiles = ['/nfs/dust/cms/user/beinsam/LongLiveTheChi/aodsim/smallchunks/pMSSM12_MCMC1_10_374794_step2_AODSIM_2of100.root']
...
# Create histograms, etc.
    ...
    hvxy                = TH1F("hvxy", "chi decay vertex xy", 120, 0, 1200)
    hvz                 = TH1F("hvz", "chi decay vertex z", 120, 0, 1200)
    hvxyz               = TH2D("hvxyz","hvxyz",120, 0, 1200, 80, 0 ,800)
    hchieta             = TH1F("hchieta", "#chi #eta distribution", 100, -3.5,3.5 )
    hDrChiTrack         = TH1F("hDrChiTrack", "min #DeltaR(#chi^{#pm},track)", 50, -0.00001, .01)
    ...
    hlog10ctau          = TH1F("hlog10ctau", "#chi^{#pm} #DeltaS (gen level)", 45,-1,4.5)
    hlog10ctauTag       = TH1F("hlog10ctauTag", "#chi^{#pm} #DeltaS(tagged tracks)", 45,-1,4.5)
...
#            if gp.pt() < 30 :continue # or abs(gp.eta()) > 1.0):continue
            if abs(gp.pdgId())==1000024 and gp.status()==1:
               ...
               # if ((abs(gp.daughter(0).pdgId()) == 1000022 and abs(gp.daughter(1).pdgId()) == 211) or (abs(gp.daughter(1).pdgId()) == 1000022 and abs(gp.daughter(0).pdgId()) == 211)):  #new way
                try:  # old way
                    print 10*'*'
                    print gp.daughter(0).pdgId()
                    print gp.daughter(0).vx()
                    print gp.daughter(0).pt()
                    print gp.daughter(1).pdgId() #, gp.daughter(2).pdgId()
                    print gp.daughter(1).vx()
                    print gp.daughter(1).pt()
                    print gp.daughter(2).pdgId()
                    print gp.daughter(2).vx()
                    print gp.daughter(2).pt()
                    print 10*'*'
                    if ((abs(gp.daughter(0).pdgId()) == 1000022 and abs(gp.daughter(1).pdgId()) == 211) or (abs(gp.daughter(1).pdgId()) == 1000022 and abs(gp.daughter(0).pdgId()) == 211)):

                        log10decaylength = TMath.Log10((TMath.Sqrt(pow(gp.daughter(0).vx() - gp.vx(),2) + pow(gp.daughter(0).vy()-gp.vy(),2) ))) #TMath.Log10
                        vxy = (TMath.Sqrt(pow(gp.daughter(0).vx(),2) + pow(gp.daughter(0).vy(),2)))
                        print gp.daughter(0).pdgId() #, gp.daughter(1).pdgId(), gp.daughter(2).pdgId()
                        print gp.daughter(1).pdgId()
                        #                    if abs(gp.daughter(0).pdgId()) == 1000022: hdaughterDM.Fill(3)
                        #                   if not(abs(gp.daughter(0).pdgId()) == 1000022): 
                        vz  = (abs(gp.daughter(0).vz()))
                        print log10decaylength
                ...
                #print 'no daughters'
#                    print 'except', gp.daughter(0).pdgId()
 #                   print 'except', gp.daughter(1).pdgId()
                    log10decaylength = -1.49
                    vxy = -1
                    vz  = -1
                hvxyz.Fill(vz,vxy)
