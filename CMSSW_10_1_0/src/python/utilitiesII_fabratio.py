...
def FabDrawPrediction(cGold,legbkg, legsig,hData,hbkgstack,hsigstack,hbkg,hbkgStat,var,lumi=35.9,LinearScale=False, fraction = '#frac{prediction}{data}'):
	cGold.cd()
	...
	cGold.Update()
	hsigstack.Draw("e0 nostacksame,ep")
# 	hbkg.Draw("e2")
#	hbkg.SetLineStyle(1)
...
#	hRatio.Divide(hDataCopy)
	hbkgCopyStat = hbkgStat.Clone()
	hbkgCopy = hbkg.Clone()   # hist with stat ans sys uncertainty on background
	hRatio = hData.Clone('hRatio')
	hRatio.Divide(hbkgCopy) # makes ratio with total uncertainty
#	hRatio.Divide(hbkgCopyStat)  # makes ratio with statistical uncertainty in data and bkg
#	hRatio.GetYaxis().SetRangeUser(0.0,.1)
	...
	hRatio.SetMarkerStyle(20)
        ...
        hRatio.Draw('e0')
	cGold.Update()
	hRatiosysUp = hbkg.Clone('hRatiosysUp')
	hRatiosysDown = hbkg.Clone('hRatiosysDown')
	hRatiosys = hbkg.Clone('hRatiosys')
	hRatiosysUp.Reset()
	hRatiosysDown.Reset()
	hRatiosys.Reset()
	hRatiosysUp.SetLineColor(kWhite)
	hRatiosysDown.SetLineColor(kWhite)
	hRatiosys.SetLineColor(kBlack)
	hRatiosysUp.SetFillStyle(3001)
	hRatiosysDown.SetFillStyle(3001)
	hRatiosys.SetFillStyle(1)
	hRatiosysUp.SetFillColor(49)
	hRatiosysDown.SetFillColor(10)


	x_axis = hData.GetXaxis()
	for ibin in range(1,x_axis.GetNbins()+1):
		if hbkgCopy.GetBinContent(ibin) > 0 : fracerr = hbkgCopy.GetBinError(ibin)/hbkgCopy.GetBinContent(ibin)
		else: fracerr = 0
		sig = 0
		if hbkgCopy.GetBinContent(ibin) > 0 and hData.GetBinContent(ibin) > 0:
			sig = (hData.GetBinContent(ibin)-hbkgCopy.GetBinContent(ibin))/math.sqrt(hbkgCopy.GetBinContent(ibin))
			print '#'*10
			print 'bin number:',ibin, 'Signifance:', sig
			print '#'*10
			pause()

		print 'Bin:',ibin,'content:',hbkgCopy.GetBinContent(ibin),'error:',hbkgCopy.GetBinError(ibin),'fracerr:',fracerr
		hRatiosysUp.SetBinContent(ibin,2+2*fracerr)
		hRatiosysDown.SetBinContent(ibin,2-2*fracerr)
		print 'sysUp:',hRatiosysUp.GetBinContent(ibin),'sysDown:',hRatiosysDown.GetBinContent(ibin)
		hRatiosys.SetBinContent(ibin,1)
		hRatiosysUp.SetBinError(ibin,0)
		hRatiosysDown.SetBinError(ibin,0)
		hRatiosys.SetBinError(ibin,0)
	for ibin in range(1,x_axis.GetNbins()+1):
		print 'sysUp:',hRatiosysUp.GetBinContent(ibin),'sysDown:',hRatiosysDown.GetBinContent(ibin)
	pause()

	hRatio.GetYaxis().SetRangeUser(-0.2,2.7)
	hRatiosysUp.Scale(0.5)
	hRatiosysDown.Scale(0.5)
	hRatio.Draw('e0')
	hRatiosysUp.Draw('same')
	hRatiosysDown.Draw('same')
	hRatio.Draw('e0 same')
#	hRatio.Draw('axis same')
	cGold.Update()
	pause()
	return hRatio,hbkg_,hRatiosysUp,hRatiosysDown
