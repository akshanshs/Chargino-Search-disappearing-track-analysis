...
fname = '/nfs/dust/cms/user/singha/LLCh/BACKGROUNDII/CMSSW_8_0_20/src/MCTemplatesBinned/BinnedTemplatesII.root'
fSmear  = TFile(fname)
	...
	for ientry in range(nentries):
		...
		for igen, gen in enumerate(c.GenParticles):
				if 1 < 2:
						...
						if not (abs(gen.Eta()) < 2.4): continue
						...
						if drsmall < .01:
							...
							#	sf = 1 #0**(sfHist.GetRandom())
								...
								if abs(c.Electrons[idlep].Eta()) < 1.4442:fillth1(hEleGenPtbarrelRECOeff, smearedEle.Pt(), weight)
								if abs(c.Electrons[idlep].Eta()) > 1.4442:fillth1(hEleGenPtECRECOeff, smearedEle.Pt(), weight)
						...
						if drsmal < .01:
								...
								if abs(trk.Eta()) < 1.4442: fillth1(hEleGenPtbarrelDTeff, c.tracks[idtrk].Pt(), weight)
								if abs(trk.Eta()) > 1.4442: fillth1(hEleGenPtECDTeff, c.tracks[idtrk].Pt(), weight)
		...
		for ireco, reco in enumerate(c.Electrons):
				if not (abs(reco.Eta()) < 2.4): continue#< 1.4442)): continue
