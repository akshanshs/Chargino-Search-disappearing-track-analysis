...
#        if TriggerResult(MetTriggers) == False: continue
	...
	disappearingTracks = []	
        tauTracks = []
	for itrack, track in enumerate(c.chiCands):
		...
		if not (abs(track.Eta()) > 1.566 or abs(track.Eta()) < 1.4442): continue
                if not isBaselineTrack_sig(track, itrack, c, hMask): continue
		basicTracks.append([track,1, itrack])
		if not (track.Pt() > Pt_threshold and track.Pt()<9999): continue		
		if not isDisappearingTrack_sig_(track, itrack, c): continue  # in principle it is N minus one disappearing track 
		drlep = 99
