...
if 'g2600' in identifier:
    weight = 0.0000462
elif 'g2500' in identifier:
    weight = 0.0000768
elif 'g2400' in identifier:
    weight = 0.000128
elif 'g2300' in identifier:
    weight = 0.000213
elif 'g2200' in identifier:
    weight = 0.000356
elif 'g2000' in identifier:
    weight = 0.00101
elif 'g2100' in identifier:
    weight = 0.000598
elif 'g1900' in identifier:
    weight = 0.00171
elif 'g1800' in identifier:
    weight = 0.00293
elif 'g1850' in identifier:
    weight = 0.0024
elif 'g1700' in identifier:
    weight = 0.00507
elif 'g1600' in identifier:
    weight = 0.00887
elif 'g1500' in identifier:
    weight = 0.0157
elif 'g1400' in identifier:
    weight = 0.0284
elif 'g1300' in identifier:
    weight = 0.0522
elif 'g1200' in identifier:
    weight = 0.0985
elif 'g1100' in identifier:
    weight = 0.191
elif 'g1000' in identifier:
    weight = 0.385
elif 'g900' in identifier:
    weight = 0.812
elif 'g800' in identifier:
    weight = 1.81
elif 'g700' in identifier:
    weight = 4.32
elif 'g550' in identifier:
    weight = 19.2


print 'weight is', weight
newfname = 'Signal_PredictionwithSYSHists_'+identifier+'.root'
...
regionCuts['SignalRegion1jet']   = [(250,inf), (250,inf), (-2.0,2), (0,inf), (-1,2.5), (1,inf), (0,inf), (0,2.4), (0,inf), (-1,inf)]
...
if prediction : hMask = fMask.Get('hEtaVsPhiDT_maskData-2016Data-2016')  #'hEtaVsPhiDT_maskRun2016')
else: hMask = ''
...
MetTriggers = [53, 54, 55, 56, 57, 58, 59, 60]
...
#        if TriggerResult(MetTriggers) == False: continue
	...
	for itrack, track in enumerate(c.tracks):
		...
		if not getGenID(track): continue     #getGenID(track) returns false if track not matched to gen chargino
                disappearingTracks.append(track)  # charge taken to be one as no branch  for charge.

        sigevent_ = len(disappearingTracks) > 0 
	if not (sigevent_): continue
	...
	if sigevent_:
		...
		fv.append(getBinNumber(fv))
                fv.append(nVtx)
                uncertainity = 0.3  # taking flat uncertanity
		k      = 1
                k_up   = 1 + uncertainity
                ...
                if k_up< 0: k_up = 0.000000001
                if k_down < 0: k_down =0.000000001
                if k_error < 0 :k_error = 0.000000001
		for regionkey in regionCuts:
                    for ivar, varname in enumerate(varlist_):
                        hname = 'Signal'+regionkey+'_'+varname
