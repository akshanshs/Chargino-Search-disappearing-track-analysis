...
except:fnameSig30 = 'g1800_chi1400_27_200970_step4_30AODScaledtest.root'
sig30 =TFile(fnameSig30)

try: fnameSig50 = sys.argv[7]
except:fnameSig50 = 'g1800_chi1400_27_200970_step4_50AODScaledtest.root'
sig50 =TFile(fnameSig50)

try: fnameSig100 = sys.argv[8]
except:fnameSig100 = 'g1800_chi1400_27_200970_step4_100AODScaledtest.root'
...
SigFiles = [[sig10,'Signal','10#times#chi^{#pm} c#tau=10 cm', 8],[sig30,'Signal','10#times#chi^{#pm} c#tau=30 cm', 6],[sig50,'Signal','10#times#chi^{#pm} c#tau=50 cm', 1],[sig100,'Signal','10#times#chi^{#pm} c#tau=100 cm',44]]
BkgFiles = [[Tau,'Tau','prompt (#tau^{#pm})',6],[Fake,'Fake','non-prompt', 7],[Mu,'Mu','prompt (#mu^{#pm})',2],[El,'El','prompt (e^{#pm})',4]]#,[Fake,'Fake','non-prompt', 7]]
...
#	infile.cd()
	...
	dBkgCompositionHist[stackhistsname] =  [THStack(stackhistsname,""), hSum]
	...
	legbkg = mklegend(x1=.6, y1=.6, x2= 0.99, y2=.89, color=kWhite) #place to be adjusted later
...
#	legname = 'distrack'
	themax = 1500*max([dSigCompositionHist[stackhistsname].GetMaximum(),dBkgCompositionHist[stackhistsname][1].GetMaximum(),hData.GetMaximum()])
	dBkgCompositionHist[stackhistsname][0].SetMinimum(0.02)
	dBkgCompositionHist[stackhistsname][0].SetMaximum(themax)
	dBkgCompositionHist[stackhistsname][1].GetYaxis().SetRangeUser(0.02,themax)
	hData.GetYaxis().SetRangeUser(0.02,themax)
	dSigCompositionHist[stackhistsname].SetMinimum(0.02)
	dSigCompositionHist[stackhistsname].SetMaximum(themax)
	c1 = mkcanvas('c_'+stackhistsname.replace('_',''))
	pause()
	hratio = FabDrawPrediction(c1,legbkg, legsig,hData,dBkgCompositionHist[stackhistsname][0],dSigCompositionHist[stackhistsname],dBkgCompositionHist[stackhistsname][1],var,lumi=35.9,LinearScale=False, fraction = '#frac{prediction}{data}')
#	pause()
